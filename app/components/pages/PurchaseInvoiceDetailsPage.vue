...
import { getPurchaseInvoiceDetail } from '~/data/purchaseInvoiceDetails'
...
const invoice = computed(() => getPurchaseInvoiceDetail(props.orderId))

const activityOpen = ref(false)
// Destructive delete → confirm modal (rule/btn-danger-confirm)
const deleteOpen = ref(false)
function confirmDelete() {
  toast.notify({ variant: 'success', title: t('Purchase invoice deleted'), rootProps: { class: 'toast-enterprise' } })
  router.push('/purchase-invoices')
}

// Tabs only appear once they have something to show.
...
const activityEntries = computed(() => [{
  ...
  activity: t('Created'),
  details: [
    { label: t('Transaction no.'), value: `${t('Purchase Invoice')} #${invoice.value.number}` },
    { label: t('Transaction date'), value: formatDateLong(invoice.value.date) },
    { label: t('Vendor'), value: invoice.value.vendor.name },
...
const jumpResults = computed(() => {
  const recent = [...purchaseInvoices].sort((a, b) => b.date.localeCompare(a.date))   // most recent first
  const q = jumpSearch.value.trim().toLowerCase()
  const matched = q
    ? recent.filter(inv => String(inv.number).toLowerCase().includes(q) || inv.vendor.name.toLowerCase().includes(q))
    : recent
  return matched.slice(0, 5)   // 5 most recent (or top 5 matches)
})
function jumpTo(id: string) { router.push(`/purchase-invoices/${id}`) }
...
function goBack() { router.push('/purchase-invoices') }
</script>
...
<template>
  <div v-if="invoice" class="detail-page">
    ...
    <header class="detail-bar">
      <div class="detail-bar-left">
        <MpTextlink id="detail-breadcrumb" as="a" class="detail-breadcrumb" @click.prevent="goBack">{{ t('Purchase invoices') }}</MpTextlink>
        <div class="detail-titlerow-left">
          <h1 class="detail-title">{{ t('Purchase Invoice') }} #{{ invoice.number }}</h1>
              ...
              <div class="detail-jump">
                ...
                <div class="detail-jump-list">
                  <MpButton
                    ...
                    @click="jumpTo(inv.id)"
                  >
                    <span class="detail-jump-item-number">{{ t('Purchase Invoice') }} #{{ inv.number }}</span>
                    <span class="detail-jump-item-customer">{{ inv.vendor.name }}</span>
                  </MpButton>
    ...
    <div class="detail-stage">
      ...
      <section class="detail-summary">
        ...
        <div class="content-list-grid">
          <div class="content-list-col">
            <ContentList :label="t('Vendor')" :value="invoice.vendor.name" />
          </div>
          <div class="content-list-col">
          ...
          <div class="detail-primary-total">
            ...
            <span class="detail-total-amount">{{ formatIDR(invoice.amount) }}</span>
          </div>
        </div>
        ...
        <div class="content-list-grid">
          ...
          <!-- col 3: references -->
          <div class="content-list-col">
            <ContentList :label="t('Transaction no.')" :value="`${t('Purchase Invoice')} #${invoice.number}`" />
      ...
      <section class="detail-notes">
        ...
        <div class="detail-totals">
          ...
          <div class="detail-total-row">
            ...
            <span class="detail-total-row-amt detail-total-row-amt--total">{{ formatIDR(invoice.amount) }}</span>
          </div>
        </div>
      </section>
      ...
      <div class="detail-footer">
        ...
        <!-- Actions primary dropdown — a purchase invoice is money we OWE a vendor,
             so the action set is kept simple (no receive-payment / tax-document flows). -->
    ...
    <ActivityLogModal
      ...
      :subject="`${t('Purchase Invoice')} #${invoice.number}`"
    ...
    <ConfirmModal
      ...
      :description="`${t('Purchase Invoice')} #${invoice.number} ${t('will be permanently deleted. This cannot be undone.')}`"
      :confirm-label="`${t('Delete')} ${t('purchase invoice')}`"
  ...
  <div v-else class="detail-notfound">
    <p class="detail-notfound-title">{{ t('Purchase invoice not found') }}</p>
    <p class="detail-notfound-desc">{{ t('This purchase invoice may have been deleted or the link is invalid.') }}</p>
    <MpButton variant="secondary" is-rounded @click="router.push('/purchase-invoices')">{{ t('Back to purchase invoices') }}</MpButton>
  </div>
</template>
