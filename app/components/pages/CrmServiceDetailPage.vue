...
/**
 * CRM — Service deal record detail (/crm/services/:id), for the custom "Service
 * deals" module. SAME layout, format, and components as the Deals record detail
 * (CrmDealDetailPage) — title bar → pipeline stepper (segmented bar) → tabs
 * (Service details · Activity · Notes · Files · ERP transactions). Only the
 * property/field set differs (service properties, no shipping/logistics fields).
 *
 * Stage flow mirrors Deals: a "Mark as completed" split button (primary + a
 * chevron stage picker for the ongoing stages + "Mark as cancelled"); a
 * cancelled record can be reopened; an archived record restored.
 */
...
// ERP transaction status → ErpStatusBadge tone (mirrors the ERP Sales index).
const ORDER_STATUS: Record<OrderStatus, { label: string; type: 'completed' | 'warning' | 'information' | 'announcement' }> = {
...
const ongoingStages = computed(() => stages.value.filter((s) => s.kind === 'open').map((s) => s.name))
const wonStage = computed(() => stages.value.find((s) => s.kind === 'won')?.name ?? 'Completed')
const lostStage = computed(() => stages.value.find((s) => s.kind === 'lost')?.name ?? 'Cancelled')

// ── State flags ──
const isArchived = computed(() => !!deal.value?.archived)
const isWon = computed(() => deal.value?.stage === wonStage.value)
const isLost = computed(() => deal.value?.stage === lostStage.value)
const isOngoing = computed(() => !isWon.value && !isLost.value)

// ── Pipeline stepper (segmented bar) — forward = open stages + won; lost node
//    replaces the won node when the record is cancelled. ──
const forwardStages = computed(() => [...ongoingStages.value, wonStage.value])
const currentStageIndex = computed(() => (deal.value ? forwardStages.value.indexOf(deal.value.stage) : -1))
const stepLabels = computed(() => [...ongoingStages.value, isLost.value ? lostStage.value : wonStage.value])
function isFilled(i: number) { return isLost.value ? i <= ongoingStages.value.length - 1 : i <= currentStageIndex.value }
function isLostNode(i: number) { return isLost.value && i === forwardStages.value.length - 1 }
function isCurrentStep(i: number) { return !isLost.value && i === currentStageIndex.value }
function isDoneStep(i: number) { return (!isLost.value && i < currentStageIndex.value) || (isLost.value && i <= ongoingStages.value.length - 1) }

// ── Activity log ──
const dealActivity = computed(() => (deal.value ? serviceActivityLog(deal.value) : []))

// ── Stage moves ──
function moveTo(stage: string): boolean {
  const d = deal.value; if (!d) return false
  moveServiceDealStage(d.id, stage)
  successToast(`${t('Stage changed to')} ${stage}`)
  return true
}
function markCompleted() { moveTo(wonStage.value) }
function onPickStage(stage: string) { moveTo(stage) }
function markCancelled() { moveTo(lostStage.value) }
function reopen() { moveTo(ongoingStages.value[0] ?? 'Inquiry') }

// ── Archive / restore / delete ──
const archiveConfirmOpen = ref(false)
function confirmArchive() { const d = deal.value; if (d) { archiveServiceDeal(d.id); successToast(t('Service deal archived')) } archiveConfirmOpen.value = false }
function onRestore() { const d = deal.value; if (d) { restoreServiceDeal(d.id); successToast(t('Service deal restored')) } }
const deleteConfirmOpen = ref(false)
function confirmDelete() {
  const d = deal.value
  deleteConfirmOpen.value = false
  if (d) { deleteServiceDeal(d.id); successToast(t('Service deal deleted')); router.push('/crm/services') }
}
...
// ── Products (Service details): search ──
const productSearch = ref('')
const visibleProducts = computed(() => {
  const list = deal.value?.products ?? []
  const q = productSearch.value.trim().toLowerCase()
  return q ? list.filter((li) => li.productName.toLowerCase().includes(q)) : list
})
...
function onUploadFiles(files: FileList) {
  ...
  for (const f of Array.from(files)) {
    d.files.push({ name: f.name, sizeKB: Math.round((f.size / 1024) * 10) / 10, uploadedBy: currentUser, uploadedAt: new Date().toISOString().slice(0, 19), src: URL.createObjectURL(f) })
    n++
  }
  persistServiceDeals()
...
function downloadFile(a: DealAttachment) {
  ...
  } else { infoToast(`${t('This demo file has no downloadable content.')}`) }
}
...
function confirmDeleteFile() {
  const d = deal.value
  if (d?.files && fileToDelete.value) {
    const idx = d.files.indexOf(fileToDelete.value)
    if (idx !== -1) { d.files.splice(idx, 1); persistServiceDeals(); successToast(t('File deleted')) }
  }
...
<template>
  <div v-if="deal" class="detail-page">
    ...
    <header class="detail-bar">
      <div class="detail-bar-left">
        <MpTextlink id="svc-breadcrumb" as="a" class="detail-breadcrumb" @click.prevent="router.push('/crm/services')">{{ t('Service deals') }}</MpTextlink>
      ...
      <div class="detail-titlerow-right">
        <!-- Ongoing: "Mark as completed" split button + inline stage picker -->
        <div v-if="!isArchived && isOngoing" class="detail-split-btn">
          <button class="btn-enterprise btn-enterprise--primary detail-split-btn__main" @click="markCompleted">{{ t('Mark as completed') }}</button>
          <MpPopover id="svc-stage-menu" is-close-on-select use-portal :is-keep-alive="false" placement="bottom-end">
            <MpPopoverTrigger>
            ...
            <MpPopoverContent :class="css({ minWidth: '200px', width: 'max-content', whiteSpace: 'nowrap' })">
              ...
              <MpPopoverList>
                <MpPopoverListItem v-for="s in ongoingStages.filter((x) => x !== deal!.stage)" :key="s" @click="onPickStage(s)">{{ s }}</MpPopoverListItem>
              </MpPopoverList>
              <div :class="css({ height: '1px', backgroundColor: 'var(--mp-border-default)', marginTop: 'var(--mp-spacing-1)', marginBottom: 'var(--mp-spacing-1)' })" />
              <MpPopoverList>
                <MpPopoverListItem @click="markCancelled">{{ t('Mark as cancelled') }}</MpPopoverListItem>
              </MpPopoverList>
            </MpPopoverContent>
          </MpPopover>
        </div>
        <!-- Cancelled: reopen -->
        <button v-else-if="!isArchived && isLost" class="btn-enterprise btn-enterprise--primary" @click="reopen">{{ t('Reopen') }}</button>
        <!-- Archived: restore -->
        ...
        <MpPopover id="svc-actions" is-close-on-select use-portal :is-keep-alive="false" placement="bottom-end">
          ...
          <MpPopoverContent :class="css({ minWidth: '180px', width: 'max-content', whiteSpace: 'nowrap' })">
            ...
            <template v-else>
              <MpPopoverList>
                <MpPopoverListItem @click="router.push(`/crm/services/${deal.id}/edit`)">{{ t('Edit') }}</MpPopoverListItem>
    ...
    <div class="detail-stage">
      ...
      <div v-if="isArchived" class="detail-banner detail-banner--muted">
        ...
        <span class="detail-banner-text">{{ t('This service deal is archived — hidden from active views and metrics. Restore it to edit or change stage.') }}</span>
      </div>
      <div v-else-if="deal.linkedTransaction" class="detail-banner">
        <MpIcon name="information" size="md" class="detail-banner-icon" />
        <span class="detail-banner-text">{{ t('This service deal is linked to') }} {{ deal.linkedTransaction.type }} #{{ deal.linkedTransaction.number }}. {{ t('Editing the service deal does not update the ERP transaction.') }}</span>
      </div>
      <div v-else-if="isLost" class="detail-banner detail-banner--warn">
        <MpIcon name="information" size="md" class="detail-banner-icon" />
        <span class="detail-banner-text">{{ t('This service deal was cancelled.') }}</span>
      </div>
        ...
        <MpTabPanels>
          ...
          <MpTabPanel value="details">
            <section class="detail-summary">
              <!-- Primary row: Customer · Contact person · emphasised Value -->
              <div class="content-list-grid">
                <div class="content-list-col">
                  <ContentList :label="t('Customer')" :value="deal.company || '—'" />
                </div>
                <div class="content-list-col deal-contact-col">
                  <ContentList :label="t('Primary contact')" :value="deal.contact || '—'" />
                </div>
                <div class="detail-primary-total">
                  <span class="detail-total-label">{{ t('Value') }}</span>
                  <span class="detail-total-amount">{{ money(deal.value) }}</span>
                </div>
              </div>
              ...
              <div class="content-list-grid">
                <div class="content-list-col">
                  <ContentList :label="t('Service type')" :value="deal.serviceType" />
                  <ContentList :label="t('Payment terms')" :value="deal.paymentTerms || '—'" />
                </div>
                <div class="content-list-col">
                  <ContentList :label="t('Transaction date')" :value="fmtDate(deal.transactionDate)" />
            ...
            <section class="detail-items-section detail-details-block">
              ...
              <table class="detail-items">
                <thead>
                  <tr>
                    <th class="detail-th">{{ t('Service item') }}</th>
                ...
                <tbody>
                  <tr v-for="(li, i) in visibleProducts" :key="li.productId + i" class="detail-item-row">
                    <td class="detail-td"><ProductCell :name="li.productName" /></td>
                    ...
                    <td class="detail-td detail-td--num">{{ money(lineSubtotal(li)) }}</td>
                  </tr>
                  <tr v-if="!visibleProducts.length">
                    <td class="detail-td detail-td--muted" colspan="6">
                      {{ deal.products?.length ? t('No products match your search.') : t('No products added to this service yet.') }}
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <section class="detail-notes detail-details-block">
              <div class="detail-notes-left">
                <ContentList v-if="deal.description" :label="t('Description')">
                  <p class="detail-note-text">{{ deal.description }}</p>
                </ContentList>
                <ContentList v-if="deal.memo" :label="t('Memo')">
                  <p class="detail-note-text">{{ deal.memo }}</p>
                </ContentList>
              </div>
              ...
              <div v-if="deal.products?.length" class="detail-totals">
                <div class="detail-total-row">
                  ...
                  <span class="detail-total-row-amt detail-total-row-amt--strong">{{ money(productsTotal) }}</span>
                </div>
                <div class="detail-total-rule" />
                <div class="detail-total-row">
                  <span class="detail-total-row-label detail-total-row-label--total">{{ t('Total') }}</span>
                  <span class="detail-total-row-amt detail-total-row-amt--total">{{ money(productsTotal) }}</span>
                </div>
              </div>
            </section>
          </MpTabPanel>
          ...
          <MpTabPanel value="notes">
            ...
            <CrmNotesPanel entity-type="service" :entity-id="deal.id" :author="currentUser" />
          </MpTabPanel>

          <!-- ── Files ── -->
          <MpTabPanel value="files">
            <h3 class="detail-tab-heading">{{ t('Attachment') }} ({{ deal.files?.length ?? 0 }})</h3>
              ...
              <tbody>
                <tr v-for="(a, i) in visibleAttachments" :key="`${a.name}-${a.uploadedAt ?? ''}-${i}`" class="detail-item-row">
                  ...
                  <td class="detail-td detail-td--actions">
                    <MpPopover :id="`svc-file-actions-${i}`" is-close-on-select use-portal :is-keep-alive="false" placement="bottom-end">
                      <MpPopoverTrigger>
            ...
            <table v-if="deal.linkedTransaction" class="detail-linked">
              ...
              <tbody>
                <tr class="detail-item-row">
                  <td class="detail-td">{{ fmtDate(deal.linkedTransaction.date) }}</td>
                  <td class="detail-td"><a class="cell-link cell-text" @click="goTransaction(deal.linkedTransaction.id)">{{ deal.linkedTransaction.number }}</a></td>
                  <td class="detail-td">{{ deal.linkedTransaction.type }}</td>
                  <td class="detail-td">{{ fmtDate(deal.linkedTransaction.dueDate) }}</td>
                  <td class="detail-td">
                    <ErpStatusBadge :status="deal.linkedTransaction.status" :label="ORDER_STATUS[deal.linkedTransaction.status]?.label ?? deal.linkedTransaction.status" :type="ORDER_STATUS[deal.linkedTransaction.status]?.type ?? 'announcement'" />
                  </td>
                  <td class="detail-td detail-td--num">{{ money(deal.linkedTransaction.balanceDue) }}</td>
                  <td class="detail-td detail-td--num">{{ money(deal.linkedTransaction.total) }}</td>
                </tr>
              </tbody>
            </table>
            <p v-else class="detail-tab-empty">{{ t('No ERP transaction linked to this service yet.') }}</p>
          </MpTabPanel>
        </MpTabPanels>
      </MpTabs>
    ...
    <ConfirmModal
      ...
      :title="t('Archive this service deal?')"
    ...
    <ConfirmModal
      ...
      :title="t('Delete file?')"
      :description="t('This file will be removed from the service deal. This cannot be undone.')"
  ...
  <div v-else class="detail-notfound">
    <p class="detail-notfound-title">{{ t('Service deal not found') }}</p>
    <p class="detail-notfound-desc">{{ t('This service deal may have been deleted or the link is invalid.') }}</p>
    <MpButton variant="secondary" is-rounded @click="router.push('/crm/services')">{{ t('Back to Service deals') }}</MpButton>
  </div>
</template>
...
/* Split button — "Mark as completed" main + chevron sharing one pill */
