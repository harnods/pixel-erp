...
const scopedReceipts = computed<Receipt[]>(() =>
  receiptsForStage('Completed', isScoped.value ? scopedWarehouseIds.value : undefined),
)
  ...
  >
    ...
    <template #filters>
      <div class="filter-left">
        <MpPopover v-if="!isScoped" id="cmp-wh-filter" is-close-on-select>
          <MpPopoverTrigger>
            <MpSelect
              id="cmp-wh-select" :placeholder="t('Warehouse')" :model-value="warehouseFilter" is-clearable
      ...
      <div class="filter-right">
        <div class="filter-btn-group">
          <MpTooltip id="tt-cmp-airene" :label="t('Ask Airene')" placement="bottom" use-portal>
          ...
          <ColumnSettingsMenu id="cmp-col-settings" :items="columnItems" :visibility="colVis" />
          <MpTooltip id="tt-cmp-export" :label="t('Export')" placement="bottom" use-portal>
            <MpButton class="filter-icon-btn" :aria-label="t('Export')"><MpIcon name="download" size="md" /></MpButton>
          </MpTooltip>
        </div>
    ...
    <template #cell-receivedQty="{ row }">
      {{ formatNum((row as unknown as Receipt).receivedQty) }}
    </template>
    <template #cell-receivedDate="{ value }">{{ formatDate(value as string) }}</template>

    <!-- ── Actions kebab ── -->
    <template #actions="{ row }">
      <MpPopover :id="`cmp-actions-${row.id}`" is-close-on-select use-portal :is-keep-alive="false" placement="bottom-end">
        <MpPopoverTrigger>
    ...
    <template #empty>
      <div class="empty-full">
        ...
        <p class="empty-full-title">{{ t('No completed receipts') }}</p>
        <p class="empty-full-desc">{{ t('Fully received purchase orders will appear here.') }}</p>
      </div>
    </template>
  </ErpTablePage>

  <!-- ── Demo scenario FAB ── -->
  <MpPopover id="cmp-demo-fab" is-close-on-select use-portal placement="top-end">
    <MpPopoverTrigger>
...
.cmp-received { display: inline-flex; align-items: center; gap: var(--mp-spacing-1\.5); justify-content: flex-end; }
.cmp-partial-tag {
  padding: 0 var(--mp-spacing-1\.5); border-radius: var(--mp-radii-full, 999px);
  background: var(--mp-background-information-subtle, #e8f1ff); color: var(--mp-text-information, #1d6fdc);
  font-size: var(--mp-font-sizes-2xs, 10px); font-weight: var(--mp-font-weights-semi-bold);
  line-height: var(--mp-line-heights-lg, 20px); text-transform: uppercase; white-space: nowrap;
}
