...
import {
  customers, products, salesQuotes,
...
const nextTxNo = computed(() => `Sales Quote #${nextQuoteNumber()}`)
...
// A sales quote is not a payment document — no withholding/deposit at create; the
...
function nextQuoteNumber(): number {
  return salesQuotes.reduce((max, q) => Math.max(max, q.number), 20089) + 1
}
function nextQuoteId(): string {
  const n = salesQuotes.reduce((max, q) => {
...
function onSave() {
  ...
  // A sales quote is lightweight — we don't persist line items into the record;
