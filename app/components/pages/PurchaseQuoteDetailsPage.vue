...
import { getPurchaseQuoteDetail } from '~/data/purchaseQuoteDetails'
...
const quote = computed(() => getPurchaseQuoteDetail(props.orderId))

const activityOpen = ref(false)
// Destructive delete → confirm modal (rule/btn-danger-confirm)
const deleteOpen = ref(false)
function confirmDelete() {
  toast.notify({ variant: 'success', title: t('Purchase quote deleted'), rootProps: { class: 'toast-enterprise' } })
...
const activityEntries = computed(() => [{
  ...
  activity: t('Created'),
  details: [
    { label: t('Transaction no.'), value: `${t('Purchase Quote')} #${quote.value.number}` },
    { label: t('Transaction date'), value: formatDateLong(quote.value.date) },
    { label: t('Vendor'), value: quote.value.vendor.name },
...
const jumpResults = computed(() => {
  const recent = [...purchaseQuotes].sort((a, b) => b.date.localeCompare(a.date))   // most recent first
...
function jumpTo(id: string) { router.push(`/purchase-quotes/${id}`) }
...
function goBack() { router.push('/purchase-quotes') }
</script>
  ...
  <div class="detail-page">
    ...
    <header class="detail-bar">
      <div class="detail-bar-left">
        <MpTextlink id="detail-breadcrumb" as="a" class="detail-breadcrumb" @click.prevent="goBack">{{ t('Purchase quotes') }}</MpTextlink>
        <div class="detail-titlerow-left">
          <h1 class="detail-title">{{ t('Purchase Quote') }} #{{ quote.number }}</h1>
              ...
              <div class="detail-jump">
                ...
                <div class="detail-jump-list">
                  <button
                    ...
                    @click="jumpTo(o.id)"
                  >
                    <span class="detail-jump-item-number">{{ t('Purchase Quote') }} #{{ o.number }}</span>
      ...
      <section class="detail-summary">
        ...
        <div class="content-list-grid">
          <div class="content-list-col">
            <ContentList :label="t('Vendor')" :value="quote.vendor.name" />
          </div>
          <div class="content-list-col">
        ...
        <div class="content-list-grid">
          ...
          <!-- col 3: references -->
          <div class="content-list-col">
            <ContentList :label="t('Transaction no.')" :value="`${t('Purchase Quote')} #${quote.number}`" />
            <ContentList :label="t('Reference no.')" :value="quote.referenceNo" />
            <ContentList :label="t('Warehouse')" :value="quote.warehouse" />
            <ContentList :label="t('Buyer')" :value="quote.buyer" />
          </div>
          <!-- col 5: tags -->
          <div class="content-list-col">
    ...
    <ActivityLogModal
      ...
      :subject="`${t('Purchase Quote')} #${quote.number}`"
    ...
    <ConfirmModal
      ...
      :title="t('Delete purchase quote?')"
      :description="`${t('Purchase Quote')} #${quote.number} ${t('will be permanently deleted. This cannot be undone.')}`"
      :confirm-label="`${t('Delete')} ${t('purchase quote')}`"
