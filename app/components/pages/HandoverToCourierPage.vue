...
// to re-route. The channel AWB stays LOCKED only while the recognised original
// courier is selected — re-routing clears it, reverting restores it.
    ...
    <header class="detail-bar">
      <div class="detail-bar-left">
        <nav class="detail-breadcrumb-trail">
          <MpButton class="detail-breadcrumb" @click="goBack">Shipping</MpButton>
        </nav>
      ...
      <template v-else>
        ...
        <div class="ho-section ho-grid">
          ...
          <MpFormControl id="ho-txno">
            <div class="ho-label-row">
              ...
              <MpButton type="button" class="ho-label-icon" aria-label="Transaction no. settings" left-icon="settings" @click="noSettingsOpen = true" />
            </div>
        ...
        <div class="ho-sku-section">
          ...
          <ScanBar placeholder="Scan tracking no., packing no., or order no." class="ns-scanbar" @scan="handleScan">
            <MpButton class="btn-enterprise btn-enterprise--secondary btn-enterprise--sm" type="button" @click="resetScan">Reset scan</MpButton>
          </ScanBar>
                  ...
                  >
                    ...
                    <td class="ho-td ho-td--input" :class="{ 'ho-td--input--error': courierError && !(courierByRow[row.id]?.trim()) }">
                      <!-- Courier is re-routable for every Ready-to-Ship row — including
                           ...
                           (D5 AC#1); it locks only once a shipment doc exists. -->
                      <MpPopover
...
/* Sales order no. / Packing no. cells — value is a link to their own detail page */
...
.ho-empty {
  ...
  border: 1px solid var(--mp-border-bold, #8c9596); border-radius: var(--mp-radii-lg);
}
