...
function saveBom(): string | null {
  if (isEditMode.value) {
    ...
    if (!res.ok) {
      toast.notify({ variant: 'error', title: 'Failed to save. This BOM is used by an active work order', maxWidth: 'max-content' })
      return null
    }
    return editingId
  }
...
function handleSave() {
  ...
  const id = saveBom()
  if (!id) return
  toast.notify({ variant: 'success', title: isEditMode.value ? 'Bill of materials changes saved' : 'Bill of materials saved' })
  router.push(`/bill-of-materials/${id}`)
}
function handleSaveDraft() {
  const id = saveBom()
  if (!id) return
  toast.notify({ variant: 'success', title: isEditMode.value ? 'Bill of materials changes saved' : 'Bill of materials saved as draft' })
  router.push(`/bill-of-materials/${id}`)
}
            ...
            <table class="bf-table">
              ...
              <tbody v-for="group in costGroups" :key="group.key">
                ...
                <tr v-for="row in group.rows" :key="row.id" class="bf-tr">
                  <td class="bf-td bf-td--input">
                    <MpAutocomplete :id="`cost-acc-${group.key}-${row.id}`" v-model="row.account" :data="COST_ACCOUNT_OPTIONS" label-prop="name" value-prop="id" placeholder="Select cost account" is-searchable is-clearable use-portal is-full-width @update:model-value="() => onCostAccount(group, row)" />
                  </td>
                  <td class="bf-td bf-td--input">
