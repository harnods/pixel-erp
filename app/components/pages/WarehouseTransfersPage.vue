...
import {
  MpIcon, MpTooltip, MpSelect, MpButton, MpPopover, MpPopoverTrigger, MpPopoverContent, MpPopoverList, MpPopoverListItem,
  ...
  >
    ...
    <template #filters>
      <div class="filter-left">
        ...
        <MpButton class="filter-all-btn" variant="ghost">
      ...
      <div class="filter-right">
        <div class="filter-btn-group">
          ...
          <MpTooltip id="tt-wt-export" :label="t('Export')" placement="bottom" use-portal>
            <MpButton class="filter-icon-btn" variant="ghost" :aria-label="t('Export')"><MpIcon name="download" size="md" /></MpButton>
          </MpTooltip>
        </div>
    ...
    <template #actions="{ row }">
      ...
      <div v-if="isAwaiting && viewAs === 'manager'" class="wt-approval-actions">
        ...
        <MpTooltip :id="`wt-tt-log-${row.id}`" :label="t('Approval log')" placement="top" use-portal>
          <MpButton class="row-icon-ghost" variant="ghost" :aria-label="t('Approval log')" @click.stop="openApprovalLog(row as unknown as WarehouseTransfer)">
            <MpIcon name="task-todo" size="md" />
          </MpButton>
        </MpTooltip>
        <MpTooltip :id="`wt-tt-comment-${row.id}`" :label="t('Comments')" placement="top" use-portal>
          <MpButton class="row-icon-ghost" variant="ghost" :aria-label="t('Comments')" @click.stop>
      ...
      <div v-else-if="isAwaiting" class="wt-approval-actions">
        ...
        <MpTooltip :id="`wt-tt-view-${row.id}`" :label="t('View details')" placement="top" use-portal>
          <MpButton class="row-icon-ghost" variant="ghost" :aria-label="t('View details')" @click.stop="viewDetails(row as unknown as WarehouseTransfer)">
    ...
    <template #empty>
      <div class="empty-full">
        ...
        <MpButton class="btn-enterprise btn-enterprise--secondary btn-enterprise--icon-before empty-full-cta" variant="secondary" @click="newTransfer">
    ...
    <MpModalContent>
      ...
      <MpModalFooter>
        <div class="modal-footer-btns">
          <MpButton class="btn-enterprise btn-enterprise--ghost" variant="ghost" @click="bulkCancelOpen = false">{{ t('Keep') }} {{ bulkCancelIds.length > 1 ? t('transfers') : t('transfer') }}</MpButton>
          <MpButton class="btn-enterprise btn-enterprise--danger" variant="danger" @click="confirmBulkCancel">{{ t('Cancel') }} {{ bulkCancelIds.length > 1 ? t('transfers') : t('transfer') }}</MpButton>
        </div>
      </MpModalFooter>
    </MpModalContent>
    <MpModalOverlay />
