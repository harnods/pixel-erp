...
import {
  ...
  MpPopoverList, MpPopoverListItem, MpIcon, MpTooltip, MpCheckbox, MpButton, css, toast,
} from '@mekari/pixel3'
...
onMounted(() => {
  ...
  if (route.query.saved === '1') {
    toast.notify({ variant: 'success', title: t('Shipment created') , maxWidth: 'max-content'})
...
const baseRows = computed<Row[]>(() => {
  ...
  return listShipments(isScoped.value ? scopedWarehouseIds.value : undefined).map(h => ({
    shipmentSeq: h.shipmentSeq,
...
function openComplete(row: Row) {
  ...
  if (row.needsCancelAck) {
    toast.notify({ variant: 'error', title: t('Acknowledge the canceled order before completing this shipment'), maxWidth: 'max-content' })
    return
  }
  ...
  >
    ...
    <template #filters>
      <div class="filter-left">
        <MpPopover v-if="!isScoped" id="shp-wh-filter" :is-close-on-select="false">
          <MpPopoverTrigger>
            <MpSelect
              id="shp-wh-select" :placeholder="t('Warehouse')"
        ...
        <MpPopover id="shp-status-filter" :is-close-on-select="false">
          <MpPopoverTrigger>
            <MpSelect
              id="shp-status-select" :placeholder="t('Status')" :model-value="statusFilter.length ? 'set' : ''" is-clearable
      ...
      <div class="filter-right">
        <div class="filter-btn-group">
          <MpTooltip id="tt-shp-airene" :label="t('Ask Airene')" placement="bottom" use-portal>
            <MpButton variant="ghost" class="filter-icon-btn filter-icon-btn--airene" :aria-label="t('Ask Airene')" @click="toggleAirene?.()">
          ...
          <ColumnSettingsMenu id="shp-col-settings" :items="columnItems" :visibility="colVis" />
          <MpTooltip id="tt-shp-export" :label="t('Export')" placement="bottom" use-portal>
            <MpButton variant="ghost" class="filter-icon-btn" :aria-label="t('Export')" left-icon="download" />
          </MpTooltip>
        </div>
    ...
    <template #actions="{ row }">
      <MpPopover :id="`shp-actions-${(row as unknown as Row).shipmentSeq}`" is-close-on-select use-portal :is-keep-alive="false" placement="bottom-end">
        <MpPopoverTrigger>
        ...
        <MpPopoverContent :class="css({ minWidth: '160px', width: 'max-content', whiteSpace: 'nowrap' })">
          <MpPopoverList>
            <MpPopoverListItem @click="viewDetails(row as unknown as Row)">{{ t('View details') }}</MpPopoverListItem>
            <MpPopoverListItem
              v-if="(row as unknown as Row).status === 'open'"
              @click="openComplete(row as unknown as Row)"
            >{{ t('Complete shipment') }}</MpPopoverListItem>
          </MpPopoverList>
        </MpPopoverContent>
      </MpPopover>
    </template>
    ...
    <template #empty>
      <div class="empty-full">
        ...
        <p class="empty-full-title">{{ t('No shipments') }}</p>
        <p class="empty-full-desc">{{ t('Deliveries handed over to a courier will appear here.') }}</p>
      </div>
    </template>
  </ErpTablePage>
