...
import {
  ...
  MpModalOverlay, MpModalCloseButton, MpButton, css, toast,
} from '@mekari/pixel3'
    ...
    <template #cell-salesNos="{ value, row }">
      <span class="pick-orders">
        <template v-if="expandedRows.has((row as unknown as PickingTask).id)">
          ...
          <MpButton class="pick-orders__toggle" variant="secondary" @click.stop="toggleExpand((row as unknown as PickingTask).id)">{{ t('Show less') }}</MpButton>
        </template>
        <template v-else>
          ...
          >+{{ (value as string[]).length - 1 }} {{ t('more') }}</MpButton>
        </template>
      </span>
    </template>
    ...
    <MpModalContent>
      ...
      <MpModalFooter>
        <div class="modal-footer-btns">
          <MpButton class="btn-enterprise btn-enterprise--secondary" variant="secondary" @click="closeCancelModal">{{ t('Keep task') }}</MpButton>
          <MpButton class="btn-enterprise btn-enterprise--danger" variant="danger" @click="confirmCancelTask">{{ t('Cancel task') }}</MpButton>
        </div>
      </MpModalFooter>
    </MpModalContent>
    <MpModalOverlay />
...
.pick-icons-cell { display: flex; align-items: center; justify-content: flex-start; gap: var(--mp-spacing-2); }
