...
interface CountRow { sku: string; counted: string; countedError: boolean }
...
function applyPicker(skus: string[]) {
  ...
  rows.value = skus.map(sku => existing.get(sku) ?? { sku, counted: '', countedError: false })
}
function removeRow(sku: string) { rows.value = rows.value.filter(r => r.sku !== sku) }

function setCounted(row: CountRow, val: string) {
  const n = Math.max(0, Math.floor(Number(val) || 0))
  row.counted = val === '' ? '' : String(n)
  row.countedError = false
}
function isCounted(row: CountRow) { return row.counted.trim() !== '' }
function differenceOf(row: CountRow): number | null { return isCounted(row) ? Number(row.counted) - onHandFor(row.sku) : null }
...
function handleSave() {
  ...
  const lines = rows.value.map(r => ({ sku: r.sku, qty: isCounted(r) ? Number(r.counted) : onHandFor(r.sku) }))
            ...
            <table class="scf-table">
              ...
              <tbody>
                <tr v-for="row in displayRows" :key="row.sku" class="scf-tr">
                  ...
                  <td class="scf-td scf-td--input">
                    <input :id="`scf-counted-${row.sku}`" class="scf-qty-input" type="number" min="0" :value="row.counted" placeholder="0" @input="setCounted(row, ($event.target as HTMLInputElement).value)" />
                  </td>
                  <td class="scf-td scf-td--num" :class="{ 'scf-diff--pos': (differenceOf(row) ?? 0) > 0, 'scf-diff--neg': (differenceOf(row) ?? 0) < 0 }">{{ diffLabel(row) }}</td>
                  <td class="scf-td scf-td--muted">{{ unitFor(row.sku) }}</td>
                  <td class="scf-td scf-td--num">{{ avgCostFor(row.sku).toLocaleString('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 2 }) }}</td>
...
.scf-progress-btn { display: inline-flex; align-items: center; gap: var(--mp-spacing-2); padding: var(--mp-spacing-2) var(--mp-spacing-3); border: 1px solid var(--mp-border-bold); border-radius: var(--mp-radii-md); background: var(--mp-background-neutral); font-size: var(--mp-font-sizes-md); color: var(--mp-text-default); cursor: pointer; }
...
.scf-td { padding: 8px var(--mp-spacing-4) 8px var(--mp-spacing-2); font-size: var(--mp-font-sizes-md); color: var(--mp-text-default); border-bottom: 1px solid var(--mp-border-default); vertical-align: middle; background: var(--mp-background-neutral); }
