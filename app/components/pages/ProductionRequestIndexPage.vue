...
import {
  ...
  MpPopoverList, MpPopoverListItem, MpIcon, MpSkeleton, MpButton, css, toast,
} from '@mekari/pixel3'
...
function notifySoon(label: string) {
  toast.notify({ variant: 'information', title: `${t(label)} — ${t('coming soon')}` })
}
...
function confirmReject({ rejectedQty, reason }: { rejectedQty: number; reason: string }) {
  ...
  toast.notify({ variant: 'success', title: t('Production request rejected') })
}
...
<template>
  <div class="pr-page">
    ...
    <div class="filter-bar">
      ...
      <div class="filter-right">
        <div class="filter-btn-group">
          <span class="pr-tt" :data-tooltip="t('Ask Airene')">
            <MpButton class="filter-icon-btn filter-icon-btn--airene" variant="secondary" :aria-label="t('Ask Airene')" @click="toggleAirene?.()">
          ...
          />
          <span class="pr-tt" :data-tooltip="t('Export')">
            <MpButton class="filter-icon-btn" variant="secondary" :aria-label="t('Export')" @click="notifySoon('Export')">
    ...
    <div v-if="loading || pagedProducts.length" class="pr-table-section">
      ...
      >
        <table ref="tableEl" class="pr-table">
          ...
          <thead>
            <tr>
              <th class="pr-th" colspan="2">{{ t('Product/Request') }}</th>
              <th v-if="salesOrderVisible" class="pr-th">{{ t('Sales order no.') }}</th>
              <th
              ...
              <tr class="pr-parent" :class="{ 'pr-parent--open': isOpen(p.id) }">
                ...
                <td v-if="hasActions" class="pr-td pr-td--actions">
                  <MpPopover :id="`pr-actions-${p.id}`" is-close-on-select use-portal :is-keep-alive="false" placement="bottom-end">
                    <MpPopoverTrigger>
                      <MpButton class="row-kebab" variant="secondary" :aria-label="t('More actions')" @click.stop>
                    ...
                    <MpPopoverContent :class="css({ minWidth: '180px', width: 'max-content', whiteSpace: 'nowrap' })">
                      <MpPopoverList>
                        ...
                        <MpPopoverListItem @click="createBulkWorkOrder(p)">{{ t('Bulk create work order') }}</MpPopoverListItem>
                      </MpPopoverList>
                    </MpPopoverContent>
                  </MpPopover>
                </td>
                ...
                <template v-for="(src, si) in p.sources" :key="src.sourceNo">
                  ...
                  >
                    ...
                    >
                      ...
                      <span class="pr-tt pr-tt--top" :data-tooltip="t('View attachment')">
                        <MpButton class="pr-source-attach" variant="secondary" :aria-label="t('View attachment')" @click.stop="openAttachment">
                    ...
                    <td v-if="hasActions" class="pr-td pr-td--actions pr-child-td">
                      <MpPopover :id="`pr-actions-${r.requestNo}`" is-close-on-select use-portal :is-keep-alive="false" placement="bottom-end">
                        ...
                        <MpPopoverContent :class="css({ minWidth: '160px', width: 'max-content', whiteSpace: 'nowrap' })">
                          <MpPopoverList>
                            <MpPopoverListItem @click="createWorkOrder(p, src, r)">{{ t('Create work order') }}</MpPopoverListItem>
                            <MpPopoverListItem v-if="prChildRemaining(r) > 0" @click="openRejectModal(p, src, r)">{{ t('Reject') }}</MpPopoverListItem>
                          </MpPopoverList>
                        </MpPopoverContent>
                      </MpPopover>
                    </td>
    ...
    <div v-else class="empty-full">
      ...
      <p class="empty-full-title">{{ hasActiveFilter ? (search ? `"${search}" ${t('not found')}` : t('No production requests match your filters')) : t('No production requests') }}</p>
      <p class="empty-full-desc">{{ hasActiveFilter ? (search ? t('Recheck the keywords you have typed and try searching again.') : t('Recheck the filters you have applied and try filtering again.')) : t('Production request will appear here.') }}</p>
      <a v-if="hasActiveFilter" class="empty-clear" @click="clearFilters">{{ t('Clear filters') }}</a>
    </div>
  </div>
  ...
  <MpPopover id="pr-demo-fab" is-close-on-select use-portal placement="top-end">
    <MpPopoverTrigger>
      <MpButton class="demo-fab" variant="secondary" :aria-label="t('Change scenario state')"><MpIcon name="sliders" size="md" color="icon.inverse" /></MpButton>
    </MpPopoverTrigger>
