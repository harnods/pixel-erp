...
function confirmArchive() {
  ...
  toast.notify({ variant: 'success', title: `${warehouseToArchive.value.name} ${t('archived')}` , maxWidth: 'max-content'})
  closeArchiveModal()
}
...
function unarchive(row: Warehouse) {
  ...
  toast.notify({ variant: 'success', title: `${row.name} ${t('unarchived')}` , maxWidth: 'max-content'})
}
...
function confirmBulkArchive() {
  ...
  if (!res.ok) {
    ...
    toast.notify({ variant: 'error', title: `${t("Warehouse still has stock or open tasks and can't be archived")}: ${blocked}` , maxWidth: 'max-content'})
    bulkArchiveDeselect?.()
    closeBulkArchiveModal()
    return
  }
  toast.notify({ variant: 'success', title: `${count} ${count !== 1 ? t('warehouses archived') : t('warehouse archived')}` , maxWidth: 'max-content'})
  bulkArchiveDeselect?.()
  closeBulkArchiveModal()
}
...
const exportColumnLabels: Record<string, string> = {
  name: t('Name'), code: t('Code'), skuTotal: t('SKU qty'),
  pics: t('PIC'), address: t('Address'), status: t('Status'),
}
...
const emptyTitle = computed(() => statusFilter.value === 'archived' ? t('No archived warehouses') : t('No warehouses'))
const emptyDesc = computed(() =>
  statusFilter.value === 'archived' ? t('Warehouses you archive will appear here.') : t('Warehouses will appear here once created.'),
)
</script>
...
<template>
  ...
  >
    ...
    <template #cell-name="{ value, row }">
      <div class="cell-with-action">
        ...
        <MpBadge v-if="(row as unknown as Warehouse).isDefault" for="tableStatus" type="announcement" size="sm" class="badge-wh-default">{{ t('DEFAULT') }}</MpBadge>
      </div>
    </template>
    ...
    <template #actions="{ row }">
      <MpPopover :id="`wh-actions-${row.id}`" is-close-on-select use-portal :is-keep-alive="false" placement="bottom-end">
        ...
        <MpPopoverContent :class="css({ minWidth: '160px', width: 'max-content', whiteSpace: 'nowrap' })">
          <MpPopoverList>
            <MpPopoverListItem @click="goToDetail((row as unknown as Warehouse).id)">{{ t('View details') }}</MpPopoverListItem>
            <MpPopoverListItem @click="goEdit((row as unknown as Warehouse).id)">{{ t('Edit') }}</MpPopoverListItem>
            ...
            <MpPopoverListItem @click="goConfigure((row as unknown as Warehouse).id)">{{ t('Configure warehouse') }}</MpPopoverListItem>
          </MpPopoverList>
        </MpPopoverContent>
      </MpPopover>
    </template>
  ...
  >
    <MpModalContent>
      <MpModalHeader>
        {{ t('Archive') }} {{ bulkArchiveCount }} {{ bulkArchiveCount !== 1 ? t('warehouses') : t('warehouse') }}?
        <MpModalCloseButton />
      </MpModalHeader>
      <MpModalBody>
        {{ t('Archived warehouses will no longer appear in the active list. You can unarchive them at any time.') }}
      </MpModalBody>
      <MpModalFooter>
        <div class="modal-footer-btns">
          ...
          <div class="export-section">
            ...
            <div class="export-radio-group">
              <label class="export-radio-item">
                ...
                />
                <span>{{ t('All warehouses') }} ({{ total }})</span>
              </label>
              <label class="export-radio-item">
                <MpRadio
              ...
              <label class="export-radio-item" :class="{ 'export-radio-item--disabled': selectedCount === 0 }">
                ...
                />
                <span>{{ t('Selected') }} {{ selectedCount }} {{ selectedCount !== 1 ? t('warehouses') : t('warehouse') }}</span>
              </label>
            </div>
          </div>
  ...
  >
    <MpModalContent>
      <MpModalHeader>
        {{ t('Delete') }} {{ bulkDeleteCount }} {{ bulkDeleteCount !== 1 ? t('warehouses') : t('warehouse') }}?
        <MpModalCloseButton />
      </MpModalHeader>
      <MpModalBody>
        {{ t('Deleted warehouses cannot be restored.') }}
      </MpModalBody>
      <MpModalFooter>
        <div class="modal-footer-btns">
