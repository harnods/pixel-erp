...
import {
  ...
  MpModalOverlay, MpModalCloseButton, css, toast, MpButton,
} from '@mekari/pixel3'
    ...
    <template #cell-receivingTaskNos="{ value, row }">
      <span class="pa-rtasks">
        <template v-if="expandedRows.has((row as unknown as PutAwayTask).id)">
          ...
          <MpButton class="pa-rtasks__toggle" variant="secondary" @click.stop="toggleExpand((row as unknown as PutAwayTask).id)">{{ t('Show less') }}</MpButton>
        </template>
        <template v-else>
