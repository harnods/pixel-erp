...
/**
 * "New purchase quote" form — Purchases ▸ Purchase quotes ▸ New purchase quote.
 *
 * Vendor-keyed mirror of NewSalesQuotePage.vue: the page shell / header grid /
 * footer follow PurchaseOrderFormPage.vue; the line-items table, its
 * prefix-suffix cells and its error treatment follow NewExpensePage.vue.
 */
import {
...
import {
  vendors, products, purchaseQuotes,
  PAYMENT_TERMS, WAREHOUSES, UNIT_OPTIONS, TAX_OPTIONS,
} from '~/data'
import type { PurchaseQuote } from '~/data/types'
...
const nextTxNo = computed(() => `Purchase Quote #${nextQuoteNumber()}`)
...
// Picking a vendor seeds the email chip list — the only header field the
...
// A purchase quote is not a payment document — no withholding/deposit at create; the
// header figure is the quote total.
const quoteTotal = computed(() => total.value)
...
function validate(): boolean {
  let ok = true
  if (!vendorId.value) { vendorError.value = true; ok = false }
...
function nextQuoteNumber(): number {
  return purchaseQuotes.reduce((max, q) => Math.max(max, q.number), 30089) + 1
}
function nextQuoteId(): string {
  const n = purchaseQuotes.reduce((max, q) => {
    const num = parseInt(q.id.replace(/\D/g, ''), 10)
    return Number.isNaN(num) ? max : Math.max(max, num)
  }, 0) + 1
  return `PQ${String(n).padStart(3, '0')}`
}

function onCancel() { router.push('/purchase-quotes') }
...
function onSave() {
  ...
  // A purchase quote is lightweight — we don't persist line items into the record;
  // the in-form items only drive the quote total.
  const quote: PurchaseQuote = {
    id: nextQuoteId(),
    number: nextQuoteNumber(),
    vendor: { id: vendor.id, name: vendor.name },
    date: dmyToIso(txDate.value),
    expirationDate: dmyToIso(expirationDate.value || txDate.value),
    status: 'open',
    total: total.value,
    tags: tagsList.value.map(t2 => String(t2.value)),
  }
  purchaseQuotes.push(quote)
  toast.notify({ variant: 'success', title: t('Purchase quote created'), rootProps: { class: 'toast-enterprise' } })
  router.push(`/purchase-quotes/${quote.id}`)
}
</script>
  ...
  <div class="si-form-page">
    ...
    <header class="si-form-bar">
      <div class="si-form-bar-left">
        <MpTextlink id="si-crumb" as="a" class="si-crumb" @click.prevent="onCancel">{{ t('Purchase quotes') }}</MpTextlink>
        <h1 class="si-form-h1">{{ t('New purchase quote') }}</h1>
      </div>
    </header>
    ...
    <div class="si-form-stage">
      ...
      <!-- ── Header section 1: Vendor + Email + Balance due ── -->
      <section class="si-header1 si-dashed-divider">
        <MpFormControl id="f-vendor" class="si-field" is-required :is-invalid="vendorError">
          <MpFormLabel>{{ t('Vendor') }}</MpFormLabel>
          <MpAutocomplete
            id="f-vendor-inp" v-model="vendorId" :data="vendorOptions"
            label-prop="name" value-prop="id" is-searchable use-portal is-full-width
            :placeholder="t('Select vendor')" :is-invalid="vendorError"
        ...
        <div class="si-header1-total">
          <h3 class="si-header1-total-value">{{ t('Quote total') }} {{ fmt(quoteTotal) }}</h3>
        </div>
      </section>
      ...
      <section class="si-header2">
        ...
        <div class="si-header2-col">
          ...
          <MpFormControl id="f-expiration-date" class="si-field">
            <MpFormLabel>{{ t('Expiration date') }}</MpFormLabel>
            <MpDatePicker id="f-expiration-date-inp" v-model="expirationDate" class="si-datepicker" format="DD/MM/YYYY" value-type="format" use-portal />
          </MpFormControl>
...
/* ── Header section 2 — every column is a FIXED 318px (= the Vendor/Email
