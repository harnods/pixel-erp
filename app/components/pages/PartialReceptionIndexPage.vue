...
import {
  ...
  MpModalOverlay, MpModalCloseButton, MpButton, css,
} from '@mekari/pixel3'
...
function canClose(row: Receipt): boolean {
  return !isManualReceipt(row) && !receivingTasksForReceipt(row.id).some(task => task.status === 'open' || task.status === 'in progress')
}
  ...
  >
    ...
    <template #filters>
      <div class="filter-left">
        <MpPopover v-if="!isScoped" id="par-wh-filter" is-close-on-select>
          <MpPopoverTrigger>
            <MpSelect
              id="par-wh-select" :placeholder="t('Warehouse')" :model-value="warehouseFilter" is-clearable
      ...
      <div class="filter-right">
        <div class="filter-btn-group">
          <MpTooltip id="tt-par-airene" :label="t('Ask Airene')" placement="bottom" use-portal>
          ...
          <MpTooltip id="tt-par-export" :label="t('Export')" placement="bottom" use-portal>
            <MpButton class="filter-icon-btn" variant="secondary" :aria-label="t('Export')"><MpIcon name="download" size="md" /></MpButton>
          </MpTooltip>
        </div>
    ...
    <template #actions="{ row }">
      <MpPopover :id="`par-actions-${row.id}`" is-close-on-select use-portal :is-keep-alive="false" placement="bottom-end">
        <MpPopoverTrigger>
          <MpButton class="row-kebab" variant="secondary" :aria-label="t('More actions')">
        ...
        <MpPopoverContent :class="css({ minWidth: '190px', width: 'max-content', whiteSpace: 'nowrap' })">
          <MpPopoverList>
            ...
            >{{ t('Purchase receiving') }}</MpPopoverListItem>
            <MpPopoverListItem
    ...
    <template #empty>
      <div class="empty-full">
        ...
        <p class="empty-full-title">{{ t('No partial receptions') }}</p>
        <p class="empty-full-desc">{{ t('Purchase orders received short of their full quantity will appear here.') }}</p>
      </div>
    </template>
  </ErpTablePage>
    ...
    <MpModalContent>
      ...
      <MpModalBody>
        <template v-if="receiptToClose">
          {{ t('Only') }} {{ formatNum(receiptToClose.receivedQty) }} {{ t('of') }} {{ formatNum(receiptToClose.purchaseQty) }} {{ t('units were received. Closing accepts this as final — the remaining') }} {{ formatNum(outstanding(receiptToClose)) }} {{ t('units will not be received.') }}
        </template>
      </MpModalBody>
      <MpModalFooter>
        <div class="modal-footer-btns">
          <MpButton class="btn-enterprise btn-enterprise--secondary" variant="secondary" @click="closeCloseModal">{{ t('Keep open') }}</MpButton>
          <MpButton class="btn-enterprise btn-enterprise--danger" variant="danger" @click="confirmClose">{{ t('Close order') }}</MpButton>
        </div>
      </MpModalFooter>
    </MpModalContent>
    <MpModalOverlay />
