...
async function save() {
  ...
  if (isEdit.value && editingCustom.value) {
    ...
    // aren't persisted here. Two tenant-owned policies still are, each in its own
    // SKU-keyed store: tax info (so Tax/Finance can classify catalogue products
    // they can't edit) and replenishment settings (saved from the warehouse-stock
    // drawer, so seed products carry their own reorder policy too).
