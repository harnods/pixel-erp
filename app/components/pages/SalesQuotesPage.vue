...
function openCopyLinks(rs: Row[]) {
  copyItems.value = rs.map(r => ({ title: `${t('Sales Quote')} #${r.number}`, subtitle: r.customerName, url: recordLink(r.id) }))
  copyOpen.value = true
}
function openShare(r: Row) {
  shareTitle.value = `${t('Sales Quote')} #${r.number}`
  shareSubject.value = `${t('Sales Quote')} #${r.number}`
...
<template>
  ...
  <ExportModal :open="exportOpen" :title="t('Export sales quotes')" entity-label="sales quotes" :columns="exportColumns" :custom-fields="[t('Sample custom field 1'), t('Sample custom field 2')]" :total="total" @close="exportOpen = false" @export="exportOpen = false" />
