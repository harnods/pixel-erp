...
const nextTxNo = computed(() => `Sales Order #${nextOrderNumber()}`)
...
function nextOrderNumber(): number {
  return salesOrders.reduce((max, o) => Math.max(max, o.number), 10089) + 1
}
function nextOrderId(): string {
  const n = salesOrders.reduce((max, o) => {
...
function onSave() {
  ...
  const orderItems: SalesOrderItem[] = items.value.map(it => ({
    ...
    amount: Math.round(it.qty * it.unitPrice * (1 - it.discountPct / 100)),
  }))
