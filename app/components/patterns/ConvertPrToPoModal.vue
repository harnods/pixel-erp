 ...
 *
 * Custom Teleport overlay, matching CreatePurchaseRequestModal.vue — MpModal has
 * no structural CSS in this Pixel3 build.
 */
...
const summaryLine = computed(() => {
  ...
  if (poCount === 0) return t('Nothing can be ordered from this request.')
  ...
  return `${poPart} · ${vendorPart} · ${linePart}`
})
    ...
    <div v-if="isOpen" class="rp-po-overlay" @click.self="close">
      <div class="rp-po-panel" role="dialog" aria-modal="true" :aria-label="t('Create purchase order')">
        <header class="rp-po-header">
          <span class="rp-po-title">{{ t('Create purchase order') }}</span>
        ...
        <div class="rp-po-body">
          ...
          <p class="rp-po-summary rp-po-summary--muted">
            {{ t('Quantities are rounded to each vendor\'s MOQ and purchase multiplier. The order is saved as a draft for approval.') }}
          </p>

          <!-- Ship-to warehouse: one per conversion (a PO has a single ship-to). -->
          <div class="rp-po-shipto">
            <span class="rp-po-shipto-label">{{ t('Ship to') }}</span>
            <MpPopover id="cpo-warehouse" is-close-on-select use-portal :is-keep-alive="false" placement="bottom-start">
              <MpPopoverTrigger>
          ...
          <!-- One card per vendor — one draft PO each. -->
            ...
            <table class="rp-po-table">
              <thead>
                <tr>
                  ...
                  <th class="rp-po-th rp-po-th--num">{{ t('Needed') }}</th>
