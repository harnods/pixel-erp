...
<template>
  <div class="acf-content">
    ...
    >
      <MpPopoverTrigger>
        <button class="acf-prefix" type="button" @click.stop="open = !open">
...
.acf-prefix {
  ...
  flex-shrink: 0;
  padding: var(--mp-spacing-2, 6px);
  background: var(--mp-background-neutral-subtle, #f0f1f3);
  border: none;
  /* Rounded on the left only — it sits flush against the value input on the
     right, not floating as a standalone pill. */
  border-radius: var(--mp-radii-sm, 4px) 0 0 var(--mp-radii-sm, 4px);
...
.acf-prefix:hover { background: var(--mp-background-neutral-hovered); }
