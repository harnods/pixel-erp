...
<template>
  ...
  <div v-if="open" class="psn-overlay" @click.self="handleCancel">
    <div class="psn-panel" role="dialog" aria-label="Manage serial number">

      <header class="psn-header">
        <h2 class="psn-title">Manage serial number</h2>
      ...
      <div class="psn-content">
        ...
        <div class="psn-columns">
          ...
          <div class="psn-col">
            ...
            <div class="psn-col-head">
              <span class="psn-col-title">Selected serial number ({{ selected.length }})</span>
              <a class="psn-link" :class="{ 'psn-link--disabled': !selected.length }" @click.prevent="removeAll">Remove all</a>
            </div>
            <div class="psn-list">
              <div v-for="s in filteredSelected" :key="s" class="psn-row">
                <span class="psn-row-text">{{ s }}</span>
                <button class="psn-row-btn psn-row-btn--remove" type="button" :aria-label="`Remove ${s}`" @click="removeOne(s)">
                  <MpIcon name="minus-circular" size="sm" />
                </button>
              </div>
              <p v-if="!filteredSelected.length" class="psn-empty-text">No serial numbers selected.</p>
            </div>
          </div>
        </div>
      </div>
