 ...
 * (Figma: Barcode settings / Customize barcode format). Drop it next to any
 * disabled/auto-generated Barcode field; it owns its own modal, so it's a single
 * self-contained "just call it" component.
 *
 ...
 *
 * One MpModal, default centered behaviour (no custom placement), that swaps
 * between two steps:
 *   'info'      — read-only: next barcode number + a "Customize format" action
 *   'customize' — a vertical form (standard/prefix/separator/padding/start number)
 *                 with a live preview, driven by the `kind`'s own config store.
 */
import {
...
import {
  getBarcodeConfig, previewNextBarcode, setBarcodeFormat, formatBarcodeValue,
...
const step = ref<'info' | 'customize'>('info')
...
function save() {
  ...
  toast.notify({ variant: 'success', title: 'Barcode format updated', maxWidth: 'max-content' })
  close()
}
</script>
...
<template>
  ...
  >
    <MpModalContent>
      <MpModalHeader>
        {{ step === 'info' ? `${KIND_LABEL[props.kind]} barcode settings` : `Customize ${KIND_LABEL[props.kind]} barcode format` }}
        <MpModalCloseButton />
      </MpModalHeader>
      <MpModalBody>

        <!-- Step 1: info -->
        <div v-if="step === 'info'" class="bcs-info">
          <p class="bcs-text">Barcode number is auto-generated by the system.</p>
          <p class="bcs-text">Next barcode number: {{ previewNextBarcode(props.kind) }}</p>
        </div>

        <!-- Step 2: customize (vertical form) -->
        <div v-else class="bcs-form">
      ...
      <MpModalFooter>
        <div class="bcs-footer-btns">
          <button v-if="step === 'info'" class="btn-enterprise btn-enterprise--primary" @click="openCustomize">
            Customize format
          </button>
          <template v-else>
            <button class="btn-enterprise btn-enterprise--ghost" @click="close">Cancel</button>
            <button class="btn-enterprise btn-enterprise--primary" @click="save">Save changes</button>
          </template>
        </div>
      </MpModalFooter>
    </MpModalContent>
...
.bcs-info { display: flex; flex-direction: column; gap: var(--mp-spacing-2); }
.bcs-text { margin: 0; font-size: var(--mp-font-sizes-md); line-height: var(--mp-line-heights-md); color: var(--mp-text-default); }
