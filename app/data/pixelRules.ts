...
export const RULE_MEANINGS: Record<string, string> = {
  ...
  'rule/filter-bar-icon-group': 'Filter-bar tools = ghost is-rounded icon MpButtons (airene-brand, table-view-column via shared ColumnSettingsMenu, download) in one MpButtonGroup, 8px apart (default gap, never gap:0), each with a tooltip — all identical oval hover.',
  'rule/filter-all-filters-active-count': '"All filters" active = "All filters (N)" with border-bold kept (never brand/green fill); N counts active drawer filter rows (amount trio = 1).',
  ...
  'rule/journal-entry-structure': 'Body = heading + Account · Debit · Credit table with a bold Total row (debits must equal credits); Account always leads with its account no.; IDR, read-only.',
  ...
  'rule/filter-drawer-shell': 'All-filters drawer = custom Teleport shell, header "All filters", opened from the All filters button; edits a draft, commits only on Apply, ignores overlay click (form).',
  'rule/filter-drawer-fields': 'Filter fields = bold label + reused control (Keyword+scope, AdvancedDateRangePicker, MpCheckbox checklist, AmountComparatorField, typeable Tags); only the set varies.',
  'rule/filter-drawer-footer': 'Filter drawer footer = ghost "Reset filter" left, then ghost Cancel + primary Apply right; never disable Apply.',
  'rule/select-product-drawer': 'Product line-item picking = shared SelectProductDrawer (custom shell, header "Select product", ghost Cancel + primary Save); working set seeded from value, commits only on Save.',
  ...
  'rule/badge-single-mpbadge': 'One badge = MpBadge. Entity status → ErpStatusBadge (the sanctioned thin status→MpBadge mapper, not a 2nd badge); one-off → MpBadge directly. No other badge wrapper.',
  ...
  'rule/table-actions-column': 'Rightmost [...] actions column = 44px (single 38px icon button + 3px each side), always top-aligned, flush-right via flexible spacer, sticky on h-scroll; override actionsWidth only for multi-button slots.',
