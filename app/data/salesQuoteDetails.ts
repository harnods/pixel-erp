...
function buildItems(quote: SalesQuote, idx: number): SQLineItem[] {
  ...
  const portionB = preTax - portionA

  const qtyA = Math.max(1, Math.round(portionA / a.price))
  const qtyB = Math.max(1, Math.round(portionB / b.price))

  const mk = (p: typeof products[number], qty: number): SQLineItem => ({
    product: p.name,
    sku: p.code,
