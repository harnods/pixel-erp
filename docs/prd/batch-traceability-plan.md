...
- ~~**All filters drawer** `BatchTraceabilityFiltersDrawer.vue`: Vendor, Grade, Expiry / Manufacturing / Best before.~~ **Removed 2026-09-21 (product decision):** By batch is now filter-first — Product (with *All product*), Batch number (with *All batch*, narrowed to the picked products) and Warehouse, then a primary **Filter** button; nothing lists until Filter is clicked, and Filter with no product or batch shows an inline error. The attribute filters are dropped from this search (`searchBatches` still accepts them). **By transaction is filter-first too** (same date): Transaction type (with *All transaction type*) and the date are a draft that a primary **Filter** applies. Its All filters drawer (transaction number, customer, vendor, origin / destination warehouse) is **removed**, and the selection + *Batches in selected transactions* table are replaced by the By batch grouped table: a transaction parent row with its batches as expandable child rows (Attribute 1–3 labelled per line, since one transaction can hold different products). **Both modes:** Filter runs once at least one filter is set; a filter left empty means all of it.
