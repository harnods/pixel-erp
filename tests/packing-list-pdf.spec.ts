/**
 * generatePackingListPdf() builds a real, downloadable PDF document (jsPDF +
 * jspdf-autotable) — the sorting reference for "Match order" (one document per
 * packing task = one order), not a screenshot/copy of the on-screen packing
 * details page. These tests can't practically inspect the rendered PDF's pixel
 * content, so they verify the thing actually at risk of breaking: the function
 * runs without throwing across every line-item shape it has to flatten.
 */
...
describe('generatePackingListPdf', () => {
  it('generates a PDF for a plain (non-batch/serial) SKU without throwing', async () => {
    ...
    await expect(generatePackingListPdf(task, items, order)).resolves.toBeUndefined()
  })
  ...
  it('does not throw for a task with no line items, or when order info is omitted', async () => {
    ...
    await expect(generatePackingListPdf(task, [])).resolves.toBeUndefined()
  })
  ...
  it('does not throw when courier and tracking no. are provided', async () => {
    ...
    await expect(
      generatePackingListPdf(task, [], { ...order, courier: 'JNE', trackingNo: 'TRK1234567890' }),
